...
// Retrieved from http://davidcoveney.com/782/mysql-database-search-replace-with-serialized-php/
...
$host = getenv('DB_HOST').':'.getenv('DB_PORT');        // normally localhost, but not necessarily.
$usr  = getenv('DB_USER');       // your db userid
$pwd  = getenv('DB_PASS');                 // your db password
$db   = getenv('DB_NAME');           // your database

// Replace options
$replaces = array();
for ($i = 1 ; $i < count($argv) - 1; $i+=2) {
    $replaces[$argv[$i]] = $argv[$i+1];
}
//foreach ($replaces as $key=>$value) {
//    echo $key.'=>'.$value."\n";
//}
//die();
//$search_for   = $argv[1];  // the value you want to search for
//$replace_with = $argv[2];  // the value to replace it with

$cid = mysql_connect($host,$usr,$pwd);

if (!$cid) {
    echo("Connecting to DB Error: " . mysql_error() . "\n");
    die(1);
}
...
//                print_r($unserialized);
                ...
                foreach ($replaces as $search_for => $replace_with) {
    ...
    if (is_array($data)) {
        foreach ($data as $key => $value) {
            if (is_array($value)) {
                ...
                if (is_string($value)) $data[$key] = mb_str_replace($find, $replace, $value);
            }
        }
    } else {
        if (is_string($data)) $data = mb_str_replace($find, $replace, $data);
    }
